...
/*   Updated: 2025/07/01 13:22:56 by mzolotar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int check_child_fork(t_program *program, int i, t_all *all)
{
	if (all->exec->args && ft_strcmp(all->exec->args[0], ":") == 0)
	{
		if (!apply_redir(all, program))
			return (1);
		return (2);			// Exit tranquilo del hijo sin ejecutar ningún comando real
	}
	if (!apply_redir(all, program) || !apply_cmd_redir(all->exec, i, program))  // Caso normal: comando real (no ":"), aplica redirecciones
		return (1);
	return (0);
}

void	exec_and_exit_child(t_all *all, t_tokens *curr, t_program *program, int i) //🚩_NORMA
{
	int builtin_exit_code;
	int check_child_result;

	check_child_result = check_child_fork(program, i , all);
	...
	if ((is_forbidden_env_path(curr->content)) || (is_forbidden_pwd_path(curr->content)))
	{
	...
	if (check_builtin(curr->content) || must_use_env_builtin(curr->content) || must_use_pwd_builtin(curr->content))
	{
		if (!apply_cmd_redir(all->exec, i, program))
			free_and_exit_child(all, program, 1);
