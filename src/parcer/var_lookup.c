...
/*   Updated: 2025/06/19 09:12:48 by mzolotar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*expand_var(t_program *program, char *var_name)
{
	...
	if (!(var_name[0] == '_' || ft_isalpha(var_name[0])))
		return (var_name);
    found = env_key_exists(program->env, var_name); // 👇 Buscar primero en tu lista de variables
    if (found)
	{
        return (found->value);
	}
...
char	*expand_var(t_program *program, char *var_name)
{
	...
	return (""); // No encontrada, devolver cadena vacía
}
