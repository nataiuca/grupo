...
/*   Updated: 2025/07/07 13:53:37 by mzolotar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
 ...
 */
char	*build_new_content(const char *original,
		t_metachars *meta, t_program *program)
{
	int		size;
	char	*new_str;

	if (ft_strlen (original) > 2000)
		...
		t_program *program)
{
	while (tokens)
	{
		...
		{
			if (check_and_expand_var(tokens, &tokens->content, meta, program)
				== false)
				return (false);
		}
		tokens = tokens->next;
